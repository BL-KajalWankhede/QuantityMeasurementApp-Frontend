        ...
        {history.map((item) => (
          <article key={item.id} className="history-item">
            ...
            <p className="history-line">
              Input: {item.thisValue} {item.thisUnit} ({item.thisMeasurementType})
              {item.operation !== 'CONVERT' && item.thatUnit && (
                <> and {item.thatValue} {item.thatUnit} ({item.thatMeasurementType})</>
              )}
            </p>
            <p className="history-line">
              Result:{' '}
              {item.error
                ? item.errorMessage
                : item.resultString ??
                  `${item.resultValue != null ? item.resultValue.toLocaleString(undefined, { maximumFractionDigits: 4 }) : ''} ${item.resultUnit ?? ''} ${item.resultMeasurementType ?? ''}`}
            </p>
          </article>
        ))}
      </div>
